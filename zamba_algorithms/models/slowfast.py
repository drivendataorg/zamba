...
from zamba_algorithms.data.config import NEW_ZAMBA_LABELS
...
from zamba_algorithms.settings import DATA_DIRECTORY, RESULTS_DIR
...
def slowfast_zamba_finetune_mdlite_half_with_reduced_blanks():
    ...
    predict_model(data_module=zamba_data_module.test_dataloader(), trainer=trainer)


def slowfast_zamba_finetune_mdlite_predict_salonga():
    zamba_data_module = ZambaDataModule(
        batch_size=32,
        video_loader_config=VideoLoaderConfig(
            video_height=224,
        ...
        prefetch_factor=2,
        transform=slowfast_transforms(),
        predict_metadata=DATA_DIRECTORY / "processed" / "predict_salonga.csv",
    )

    predict_model(
        dataloader=zamba_data_module.predict_dataloader(),
        checkpoint=DATA_DIRECTORY
        / "results"
        / "slowfast_zamba_finetune_mdlite"
        / "version_0"
        / "checkpoints"
        / "epoch=9-step=20120-v2.ckpt",
        model_class="slowfast",
        model_name="slowfast_zamba_finetune_mdlite",
        columns=[f"species_{label}" for label in NEW_ZAMBA_LABELS if label != "unidentified"],
    )


@app.command()
def slowfast_zamba_finetuned_european():
    zamba_data_module = ZambaDataModule(
        batch_size=32,
        video_loader_config=VideoLoaderConfig(
            video_height=224,
    ...
    trainer = train_model(
        model_class="slowfast",
        model_name="slowfast_zamba_finetuned_european",
        model_params={
            "finetune_from": RESULTS_DIR
            / "slowfast_zamba_finetune_mdlite"
            / "version_0"
            / "checkpoints"
            / "epoch=9-step=20120-v2.ckpt"
