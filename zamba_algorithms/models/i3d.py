...
from zamba_algorithms.pytorch.transforms import i3d_transforms
...
@app.command()
def i3d_zamba_finetune():
    video_loader_config = VideoLoaderConfig(
        frame_indices=np.r_[
            np.arange(2, 2 + 2 * 16, 2), np.arange(2 + 2 * 16, 2 + 2 * 16 + 8 * 16, 8)
        ].tolist(),  # Like early_bias, but i3d needs > 16 frames
        video_height=224,
        video_width=224,
    ...
    zamba_data_module = ZambaDataModule(
        ...
        transform=i3d_transforms(),
    )

    trainer = train_model(
        data_module=zamba_data_module,
        model_class="i3d",
        model_name="i3d_zamba_finetune",
        model_params={
