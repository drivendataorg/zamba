...
def timesformer_transforms():
    ...
    return transforms.Compose(
        [
            ConvertTHWCtoCTHW(),
            Uint8ToFloat(),
            NormalizeVideo(mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD),
            PadDimensions((None, 8, None, None)),
        ]
    )


def x3d_transforms():
    """Transforms that correspond to the X3D model.

    .. _Torch Hub X3D documentation:
    ...
    return transforms.Compose(
        [
            ...
            Uint8ToFloat(),
            NormalizeVideo(mean=[0.45, 0.45, 0.45], std=[0.225, 0.225, 0.225]),
            PadDimensions((None, 16, None, None)),
        ]
    )
