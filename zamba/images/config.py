...
from zamba.images.bbox import BboxInputFormat, bbox_json_to_df, camtrap_dp_to_df
...
class ImageClassificationPredictConfig(ZambaImageConfig):
    ...
    def validate_save_dir(cls, values):
        ...
        if save_dir is not None and save:
            ...
            elif values["results_file_format"] == ResultsFormat.CAMTRAP_DP:
                # Camtrap DP output is a package directory named after the result stem
                save_path = save_dir / results_file_name.stem
...
class ImageClassificationTrainingConfig(ZambaImageConfig):
    ...
    Args:
        ...
        labels (Union[FilePath, DirectoryPath, pd.DataFrame]): Path to labels, or a pandas DataFrame.
            For CSV files, must contain 'filepath' and 'label' columns.
            For JSON files, must be in COCO or MegaDetector format as specified by labels_format.
            For Camtrap DP, path to a package directory, datapackage.json, or .zip archive
            (use labels_format=camtrap_dp).
        labels_format (BboxInputFormat): Format for bounding box annotations when labels are provided
            as JSON or a Camtrap DP package. Options are defined in the BboxInputFormat enum.
            Defaults to BboxInputFormat.COCO.
    ...
    data_dir: Path
    labels: Union[FilePath, DirectoryPath, pd.DataFrame]
    ...
    def validate_labels(cls, values):
        ...
        labels_format = values["labels_format"]

        if isinstance(labels, pd.DataFrame):
            return values

        labels = Path(labels)
        if labels_format == BboxInputFormat.CAMTRAP_DP:
            values["labels"] = camtrap_dp_to_df(labels)
        elif labels.is_dir():
            raise ValueError(
                "Directory labels are only supported with labels_format='camtrap_dp'."
            )
        elif labels.suffix.lower() == ".json":
            with open(labels, "r") as f:
                values["labels"] = cls.process_json_annotations(json.load(f), labels_format)
        else:
            # CSV, including compressed variants pandas infers (e.g. a zipped CSV)
            values["labels"] = pd.read_csv(labels)
