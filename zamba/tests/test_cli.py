...
def test_train_options(data_dir, sample_model_path, mocker):  # noqa: F811
    ...
    result = runner.invoke(app, ['train', '--train-data', str(data_dir), '--model-load-path', str(sample_model_path)])
    assert result.exit_code == 0
    ...
    result = runner.invoke(app, ['train', '--config', str(assets_dir / 'sample_train_config.yaml')])
    assert result.exit_code == 0
