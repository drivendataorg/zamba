...
def ensure_frame_number(arr, total_frames: Optional[int]):
    ...
    if (total_frames is None) or (arr.shape[0] == total_frames):
        ...
        raise ValueError("Cannot ensure frame number because no frames have been selected.")
...
def get_frame_time_estimates(path: os.PathLike):
    probe = ffmpeg.probe(path, show_entries="frame=best_effort_timestamp_time")
...
class VideoLoaderConfig(BaseModel):
    ...
    Args:
        crop_bottom_pixels (int, optional): Number of pixels to crop from the bottom of the video (prior to
            resizing to `video_height`).
        i_frames (bool, optional): Only load the I-Frames. See https://en.wikipedia.org/wiki/Video_compression_picture_types#Intra-coded_(I)_frames/slices_(key_frames)
        scene_threshold (float, optional): Only load frames that correspond to scene changes. See http://www.ffmpeg.org/ffmpeg-filters.html#select_002c-aselect
        megadetector_lite (float, optional): [0-1] Threshold for detection probability to use when filtering frames using megadetector lite.
        megadetector_lite_kwargs (dict, optional): Kwargs passed to MegadectectorLite.filter_frames.
        video_height (int, optional): Resize the video to this height in pixels.
        video_width (int, optional): Resize the video to this width in pixels.
        ...
        ensure_total_frames (bool): Selecting the number of frames by resampling may result in one
            ...
            is returned by either clipping or duplicating the final frame. Raises an error if no frames have been
            selected. Otherwise, return the array unchanged.
        fps (int, optional): Resample the video evenly from the entire duration to a specific number of frames per second.
        early_bias (bool, optional): Resamples to 24 fps and selects 16 frames biased toward the front (strategy used by competition winner).
        frame_indices (list(int), optional): Select specific frame numbers. Note: frame selection is done after any resampling.
        evenly_sample_total_frames (bool, optional): Reach the total number of frames specified by evenly sampling from
            the duration of the video. Defaults to False.
    """

    crop_bottom_pixels: Optional[int] = None
    i_frames: Optional[bool] = False
    scene_threshold: Optional[float] = None
    megadetector_lite: Optional[float] = None
    megadetector_lite_kwargs: Optional[dict] = None
    ...
    def check_height_and_width(cls, values):
        if (values["video_height"] is None) ^ (values["video_width"] is None):
            raise ValueError(
                f"Must provide both video_height and video_width or neither. Values provided are {values}."
            )
        return values
    ...
    def check_megadetector_lite_compatibility(cls, values):
        if values["megadetector_lite"] and (
            values["early_bias"] or values["evenly_sample_total_frames"]
        ):
            raise ValueError(
                f"megadetector_lite cannot be used with early_bias or evenly_sample_total_frames. Values provided are {values}."
            )
        return values
...
class npy_cache:
    def __init__(self, path: Optional[Path] = None):
        if path is None:
            path = Path(mkdtemp(prefix="load_video_frames_"))
    ...
    def __call__(self, f):
        def _wrapped(*args, **kwargs):
            ...
            if npy_path.exists():
                logger.debug(f"Loading {npy_path}: size {npy_path.stat().st_size}")
                return np.load(npy_path)
            else:
                logger.debug(f"Loading from disk: {vid_path}")
                loaded_video = f(*args, **kwargs)
                logger.debug(f"Writing to {npy_path}")
                np.save(npy_path, loaded_video)
                logger.debug(f"Wrote to {npy_path}: size {npy_path.stat().st_size}")
                return loaded_video
...
def load_video_frames(
    ...
    **kwargs,
    ...
    if config.video_height and config.video_width:
        pipeline = pipeline.filter("scale", f"{config.video_width},{config.video_height}")
    ...
    pipeline = pipeline.output("pipe:", format="rawvideo", pix_fmt="rgb24", **pipeline_kwargs)
    ...
    if config.megadetector_lite is not None:
        mdl = MegadetectorLite()
        detection_probs = mdl.detect_video(frames_arr=arr)
        md_kwargs = config.megadetector_lite_kwargs if config.megadetector_lite_kwargs else dict()

        # defaults if not specified in megadetector_lite_kwargs
        if not "score_threshold" in md_kwargs:
            md_kwargs["score_threshold"] = config.megadetector_lite
        if not "n_frames" in md_kwargs:
            md_kwargs["n_frames"] = config.total_frames

        arr = MegadetectorLite.filter_frames(arr, detection_probs, **md_kwargs)
