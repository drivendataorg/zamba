...
For advanced users, it is possible to fine-tune this model, making it able to
predict additional––or entirely different––class labels, but this currently
requires editing the `zamba` source code.

## What Videos Can I use?

The `zamba` models were trained using 15 second `.mp4` videos. Although videos
longer than 15 seconds will process without error, `zamba` currently **requires
 the `.mp4` format**.

 As for video resolution, this should not be an issue. `zamba` currently
...
Let's walk through a simple `zamba` use case.
...
assess the predictions made by `zamba`. In practice, your videos will
probably be named according to some timestamp or other non-class-label format!

As the video filenames suggest, we are about to make predictions on two `blank`
videos, one `elephant` video, a `small cat`, and a `large ungulate`.
...
use, and can even be used as a Python module, e.g., `import zamba`! See the
