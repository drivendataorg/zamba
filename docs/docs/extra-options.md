...
You can use a pretrained object detection model called [MegadetectorLite](models/megadetector.md#megadetectorlite) to select only the frames that are mostly likely to contain an animal. This is the default strategy for all pretrained models. The parameter `megadetector_lite_config` is used to specify any arguments that should be passed to the MegadetectorLite model. If `megadetector_lite_config` is None, the MegadetectorLite model will not be used.
...
If you are using the [MegadetectorLite](models/megadetector.md#megadetectorlite) for frame selection, there are two ways that you can specify frame resizing:
...
By default, `zamba` seeds random number generators during **inference** (video and image prediction) so repeated runs on the same inputs produce consistent results (for example, MegadetectorLite frame sampling). The inference seed defaults to `55` and can be set with the `INFERENCE_SEED` environment variable:
...
For bitwise-identical GPU results across runs (strict CUDA/cuDNN determinism, at some throughput cost on CUDA devices), set `deterministic=True` in your predict config:
