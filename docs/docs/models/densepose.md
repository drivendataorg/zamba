...
DensePose dependencies are not distributed on PyPI (so they are not installed with `pip install zamba`). Install them from GitHub after installing [PyTorch](https://pytorch.org/get-started/locally/):
